...
// const unigearLogoElement = ref(null)
// const unigearOverlay = ref(null)

// 鼠标移动事件处理
const handleMouseMove = (event) => {
  const rect = overlayImageElement.value.getBoundingClientRect()
  const x = event.clientX - rect.left
  const y = event.clientY - rect.top

  // 更新遮罩位置
  overlayImageElement.value.style.setProperty('--mask-x', `${x - 400}px`)
  overlayImageElement.value.style.setProperty('--mask-y', `${y - 400}px`)
}
const handleMouseLeave = () => {
  // 鼠标移出容器时清除圆形效果
  overlayImageElement.value.style.setProperty('--mask-x', `-9999px`)
  overlayImageElement.value.style.setProperty('--mask-y', `-9999px`)
}

// UNIGEAR Logo 鼠标事件处理
/* const handleUnigearMouseMove = (event) => {
  const rect = unigearLogoElement.value.getBoundingClientRect()
  const x = event.clientX - rect.left
  const y = event.clientY - rect.top

  // 使用 GSAP 平滑更新遮罩位置，添加轻微的缩放效果
  gsap.to(unigearOverlay.value, {
    '--mask-x': `${x - 120}px`,
    '--mask-y': `${y - 120}px`,
    duration: 0.2,
    ease: 'power2.out',
...
/* const handleUnigearMouseEnter = () => {
  // 鼠标进入时显示遮罩效果，并添加轻微的缩放动画
  gsap
    .timeline()
    .to(unigearOverlay.value, {
      opacity: 1,
      duration: 0.4,
      ease: 'power2.out',
    })
    .to(
      unigearLogoElement.value,
      {
        scale: 1.02,
        duration: 0.3,
        ease: 'power2.out',
...
<template>
  <footer class="bg-[#1e1e1e] text-white pt-15 mt-auto relative">
    <!-- 顶部分割线 -->
    <div
      class="absolute top-0 left-1/2 transform -translate-x-1/2 w-[calc(100%-2.5rem)] h-px bg-white/30"
    ></div>
    ...
    <div class="mx-auto px-5">
      ...
      <!-- Logo Section -->
      <div
        class="relative flex items-center mb-10 cursor-pointer overflow-hidden h-[249px] group"
      ...
      >
        <!-- 底图 -->
        <img
          src="/images/footer/dusk-logo.png"
          alt="Base Image"
          class="absolute inset-0 w-full h-full object-cover opacity-30"
        />
        <!-- 遮罩图 -->
        <div
          class="absolute inset-0 w-full h-full pointer-events-none bg-[url('/images/footer/d-logo.png')] bg-no-repeat bg-center bg-cover"
          style="
            mask-image: radial-gradient(
              circle 175px,
              rgba(0, 0, 0, 1) 0%,
